...
export default function Home() {
  return (
    <div className="flex flex-col mx-auto w-full items-center justify-center mt-16 px-8">
      <Image
      ...
      <div className="flex items-center gap-1 mt-8">
        {data.socials.map((link, i) => {
          if (link.href.includes("twitter")) {
            return <TwitterIcon key={i} />;
          }
          if (link.href.includes("github")) {
            return <GitHubIcon key={i} />;
          }
        })}
      </div>
    </div>
...
function LinkCard({
  ...
  return (
    <a
      href={href}
      className="flex items-center w-full max-w-3xl p-2 mb-4 rounded shadow-md bg-gray-200 hover:shadow-xl hover:scale-105 transition-all duration-300 border border-gray-300"
    >
      <div className="flex text-center w-full">
        <Image
          ...
          height={40}
        />
        <h2 className="font-semibold w-full flex justify-center items-center">
          {title}
        </h2>
      </div>
    </a>
